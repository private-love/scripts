...
import json
import global_functions2 as func
import Excel
import MySQL
								...
								while True:
									if trynum > 3:
										...
										memcache_stats_nc = nc_stdout.read()
                                						if len(memcache_stats_nc) > 10 :
                                        						memcache_stats_list = memcache_stats_nc.split("\n")
											break
										else:
											print str(host)+ "---" +str(memcache_bind_ip) + ":" + str(memcache_bind_port)+'memcache get empty'
											trynum += 1
											sleep(3)
											continue
									
								if len(memcache_stats_list) == 0:
									hit_rate = 0
                                                                        miss_rate = 0
                                                                        hit_rated = 0
                                                                        miss_rated = 0 
