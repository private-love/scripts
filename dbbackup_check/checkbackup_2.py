			...
			try:
				...
				#增加判断，直接ssh跳转，内网地址设置为192.168.0.x:port
                                ...
                                if len(hostinfo) == 2:
                                        ...
                                        port = hostinfo[1]
                                        host = ip
                                        stdin, stdout, stderr, status = tc.call('ssh ' +  str(ip) + ' -p ' + str(port) + ' \'cd ' + str(backup_dir) + ';ls -gGHrtl --full-time ' + str(file_name) + '\'')
                                else:
				...
				try:
					...
					#insert_backup_checklog_sql = "insert into check_backup_log (`bid`,`db_name`,`size`,`date`) values ('"+str(id)+"','"+str(b_name)+"','"+str(b_size)+"','"+str(Y)+"')"
                	                ...
                	                update_sql = "update `check_backup` set `size`='"+b_size_str+"',`last_check_filename`='"+str(b_name)+"',`last_check_time`='"+str(b_time)+"' where id="+str(back_id)+" limit 1"
				
				file_backup_date = str(Y)+"-"+str(m)+"-"+str(d)
				insert_backup_checklog_sql = "insert into check_backup_log (`bid`,`host_id`,`db_name`,`size`,`date`) values ('"+str(back_id)+"','"+str(id)+"','"+str(b_name)+"','"+str(b_size_str)+"','"+str(file_backup_date)+"')"
...
if __name__ == '__main__':
	...
	if len(result) > 1 :
		att_filename = "xls/backuplist_"+str(Y)+"-"+str(m)+"-"+str(d)+"_"+str(h)+str(M)+str(S)+".xlsx"
