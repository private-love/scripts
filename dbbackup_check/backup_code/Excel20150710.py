...
class Excel:
	...
	def __init__(self, filename, sheet_name='ciSheet', encoding='utf-8') :
	...
	def open_excel(self, file='system_report.xls') :
        	"""读取xls文件内容"""
        	try:
	...
	def serverlist_info_xls_write(self, file_name, sheet_name, data) :
		"""
		输出服务器信息报表
		"""
        	ezxf = xlwt.easyxf
        	...
        	self.write_xls(file_name, sheet_name, hdngs, data, heading_xf, data_xfs)

	def serverlist_info_xlsx_autofilter_write(self, file_name, sheet_name, data) :
		"""
		输出服务器信息报表，并自动归类
		"""
	...
	def write_xlsx(self, file_name, sheet_name, hdngs, data, format):
		"""
		输出xlsx格式报表
		"""
		workbook = xlsxwriter.Workbook(file_name)
