...
def check_mount_info(Y,y,m,d):
	...
	check_list = ['192.168.0.103:29622','192.168.0.108:29622','192.168.0.113:29622','192.168.0.244:29622','192.168.0.253:22','192.168.0.240:29622','180.96.68.251:29622','180.96.68.252:29622','180.96.7.115:29622','180.96.7.114:29622','180.96.7.114:29629','180.96.7.116:29629','180.96.7.116:29622','118.26.229.9:22','192.168.0.104:29622']
		...
		if str(nowday) :
			...
			try:
				...
				stdin, stdout, stderr, status = tc.call("cat /proc/mounts |awk '{print $1\" \"$2\" \"$3}'")
				data = stdout.read()
				res = data.split("\n")
				for line in res :
					if line :
						...
						if len(tmp_list) == 3 :
							src_path = tmp_list[0]
							dest_path = tmp_list[1]
							type_mount = tmp_list[2]

						list = [host+','+src_path+','+dest_path+','+type_mount]
...
if __name__ == '__main__':
	...
	if len(result) > 1 :
		att_filename = "xls/mount_list_"+str(Y)+"-"+str(m)+"-"+str(d)+"_"+str(h)+str(M)+str(S)+".xlsx"
		hdngs = ['服务器IP', '源路径','目标路径','类型']
	...
	if is_auto and isSendMail == 'yes' and len(result_mail) > 1 :
		hdngs_mail = ['服务器IP', '源路径','目标路径','类型']
		content=func.toHtml(hdngs_mail,result_mail,total_size_str)
		sub="服务器文件系统--每日统计("+str(Y)+'-'+str(m)+'-'+str(d)+")"
	...
	if len(err_result) > 0 and isSendMail == 'yes' :
		err_sub = "服务器文件系统检查存在错误("+str(Y)+'-'+str(m)+'-'+str(d)+")"
