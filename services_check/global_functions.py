...
def toHtml(hdngs_mail,list,total_size_str):
	"""备份检查内容html邮件代码转换函数"""
	#把list转换为dict
	dict = {}
    	key_list = []
	...
	total_db = 0
		...
		total_row += 1
			...
			total_db += 1
            		for i in d:
                		...
                		if (not i.strip(' ')):
                    			...
                    			if iserror or i == '0' or i == '0%' or i == '0.0%' or (i[-1] == "%" and int(float(i[0:-1])) > 80 ):
                        			html += "<td nowrap=\"nowrap\" bgcolor=\"red\" style=\"padding:0px 10px;\">"
                        			html += str(i)
                    			else:
                        			html += "<td nowrap=\"nowrap\" style=\"padding:0px 10px;\">"
        	...
        	count = True
	if total_size_str :
		html += "<tr style=\"padding:0px 10px;height:30px;\"><td>概述 </td><td colspan=\""+str(len(hdngs_mail))+"\">"+str(total_row)+"台服务器，"+str(total_db)+"个检查项</td></tr>"
...
def get_diff_time(times=1439540993):
        """
        计算时间相关天时分秒数
        """
        time_local = time.localtime(times)
        time_year = time.strftime('%Y',time_local)
        ...
        time_sec = time.strftime('%S',time_local)
        diff_day = datetime.datetime(int(time_year),int(time_month),int(time_day),int(time_hour),int(time_minute),int(time_sec))
        today = datetime.datetime.now()
        day = (today - diff_day).days
        second = (today - diff_day).seconds
        sec = second%60
        minute = second/60%60
        hour = second/60/60
        if hour>24:
